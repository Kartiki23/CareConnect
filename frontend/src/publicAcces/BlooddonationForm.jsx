...
const BloodDonationForm = ({ onClose }) => {
  ...
  useEffect(() => {
    const fetchDonationTypes = async () => {
      try {
        const res = await axios.get("http://localhost:3001/api/v1/user/donation");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await axios.post("http://localhost:3001/api/v1/user/bloodDonationForm", formData);
