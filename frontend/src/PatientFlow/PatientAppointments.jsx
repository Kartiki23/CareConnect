...
const PatientAppointments = () => {
  ...
  const handleCancel = async (appointmentId) => {
    try {
      await axios.delete(
        `https://careconnect-1-xvl2.onrender.com/api/v1/user/cancelAppointment/${appointmentId}`,
        { params: { patientId } }
      );
  ...
  const fetchUnreadCounts = async () => {
    try {
      const res = await axios.get(
        `https://careconnect-1-xvl2.onrender.com/api/v1/chat/unreadCounts/${patientId}`
      );
